...
<head>
    ...
    <title>The Monolith - Add New Coffee</title>
...
<body class="bg-[#121212] text-[#F5F5F7] min-h-screen flex items-center justify-center py-12 px-6">
    ...
    <div class="max-w-lg w-full bg-[#1A1A1A] border border-[#262626] rounded-2xl p-8 shadow-2xl">
        <div class="mb-6 text-center">
            ...
            <h2 class="serif-title text-2xl font-bold tracking-wide text-white">Add New Coffee Menu</h2>
        </div>

        <form action="/admin/products" method="POST" class="space-y-5">
            @csrf
            
            <div>
                <label class="block text-xs uppercase tracking-wider text-gray-400 font-medium mb-2">Coffee Name</label>
                <input type="text" name="name" required placeholder="e.g., Ethiopian Gesha Reserve" class="w-full bg-[#222222] border border-[#2A2A2A] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#D4AF37]/50 transition-colors">
            </div>

            <div>
                <label class="block text-xs uppercase tracking-wider text-gray-400 font-medium mb-2">Price (IDR)</label>
                <input type="number" name="price" required placeholder="e.g., 95000" class="w-full bg-[#222222] border border-[#2A2A2A] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#D4AF37]/50 transition-colors">
            </div>

            <div>
                <label class="block text-xs uppercase tracking-wider text-gray-400 font-medium mb-2">Description</label>
                <textarea name="description" rows="4" required placeholder="Jelaskan cita rasa premium, profil sangrai, atau keunikan menu kopi mewah ini..." class="w-full bg-[#222222] border border-[#2A2A2A] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#D4AF37]/50 transition-colors resize-none"></textarea>
            </div>
